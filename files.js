...
function createFile(fileName, content) {
    ...
    const {error} = dataValidate(data);
     if(error) {
        console.log(chalk.red(`Please specify ${error.details[0].context.key} parameter`))
     }
     
     const {extension, result} = checkExtension(fileName)

     if(!result) {
        console.log(chalk.red(`Application doesn't support files with ${extension} extension`))
     }
}
